...
class NMT(nn.Module):
    ...
    """
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.3, no_char_decoder=False):
        """ Init NMT Model.
        ...
        self.encoder = nn.LSTM(embed_size, self.hidden_size, bidirectional=True,
                               batch_first=True)  # lstm的参数(emdding_size,hidden_size,) 注意，时间序列不属于网络架构
        self.decoder = nn.LSTM(embed_size + self.hidden_size, self.hidden_size, batch_first=True)
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        #先把文字转为数字
        #丢进encode，这里要改一下encodding的embedding
        #同样改一下decode的embedding
        ...
        target_padded_char = self.vocab.tgt.to_input_tensor_char(target, device=self.device)#(batch, length_sentence, m_word)
        ...
        enc_masks = self.generate_sent_masks(enc_hiddens, source_lengths)
        combine_outputs = self.decode(enc_hiddens, enc_masks, dec_init_state, target_padded_char)
        ...
        ### END YOUR CODE
        proj = self.target_vocab_projection(combine_outputs)

        P = F.log_softmax(proj, dim=-1)
       ...
       #
        ...
        target_gold_words_log_prob = torch.gather(P.squeeze(2), index=target_padded[:].unsqueeze(-1),
                                                  dim=-1).squeeze(-1) * target_masks[:]  # (sequence=1, len(tgt_vocab)
        # Compute log probability of generating true target words
        #target_gold_words_log_prob = torch.gather(P.squeeze(dim=2), index=target_padded[1:].unsqueeze(-1),dim=-1).squeeze(-1) * target_masks[1:]
        scores = target_gold_words_log_prob.sum() # mhahn2 Small modification from A4 code.



        if self.charDecoder is not None:
            max_word_len = target_padded_char.shape[-1]

            target_words = target_padded[1:].contiguous().view(-1)  #8-26如果target_padded是(sequence_length, batch_size, word_emb)
            #target_padded_char:(batch, sentence_len, m_word)
            target_chars = target_padded_char[:].view(-1, max_word_len)
            target_outputs = combine_outputs.view(-1, 256)
            ...
            #把每一个词都丢到char_decoder里面去训练
            #input:char_sequence:(length, batch), h_0(1, batch, hidden_size)
    ...
    def encode(self, source_padded_char: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        x = self.model_embeddings_source(source_padded_char)  # permute前：(inputshape(scr_len,batch_size),embedding_size)

        x = pack_padded_sequence(x, source_lengths, batch_first=True)  # 打包(batch,len,embdding_size)



        enc_hiddens, (last_hidden, last_cell) = self.encoder(x)  # x,output(batch, seq, feature) ？？？这里是否要写入变量hidden和cell呢
        enc_hiddens = pad_packed_sequence(enc_hiddens, batch_first=True)[0]  # 所有的hidden 都在这里了
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
                dec_init_state: Tuple[torch.Tensor, torch.Tensor], target_padded_char: torch.Tensor) -> torch.Tensor:
        ...
        target_padded = target_padded_char[:-1]
        ...
        enc_hiddens_proj = self.att_projection(enc_hiddens)  # 再点乘dec_hidden得到attention score

        y = self.model_embeddings_target(target_padded_char) # (batch, sequence, embdding_size)


        for t in range(len(y[1])):
            y_t = torch.split(y, 1, dim=1)[t]  # 一个词一个词丢进去（batch, 1, embedding_size）
            y_t = torch.squeeze(y_t, dim=1)  # (batch, embedding_size_)

            ybar_t = torch.cat((y_t, o_prev), 1)  # (batch_size, embedding_size + hidden_size)



            dec_state, combined_output, e_t = self.step(ybar_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks)

            combined_outputs.append(combined_output)#希望的输出是(batch, hidden_size)
            o_prev = combined_output

        combined_outputs = torch.stack(combined_outputs, dim=0).unsqueeze(dim=2)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        ### COPY OVER YOUR CODE FROM ASSIGNMENT 4
        output, dec_state = self.decoder(Ybar_t.unsqueeze(dim=1), dec_state)  # 输入Ybar_t必须是三个维(batch, seq, feature)

        dec_hidden, dec_cell = dec_state[0], dec_state[1]  # size:(sequence = 1, batch, hidden_size)
        dec_state = (dec_state[0], dec_state[1])

        e_t = torch.bmm(enc_hiddens_proj, dec_hidden.squeeze(dim=0).unsqueeze(dim=2)).squeeze(2)  # enc_hiddens_proj:(batch_size, encode_sequence, hidden_size)
        ...
        ### COPY OVER YOUR CODE FROM ASSIGNMENT 4
        alpha_t = F.softmax(e_t, dim=1).unsqueeze(1)  # (batch, decoder_sequence=1, encoder_sequence)

        a_t = torch.bmm(alpha_t,
                        enc_hiddens)  # (batch, 1, sequence).*(batch, sequence, hidden*2) = (batch, 1, hidden*2)
    ...
    def beam_search(self, src_sent: List[str], beam_size: int=5, max_decoding_time_step: int=70) -> List[Hypothesis]:
        ...
        while len(completed_hypotheses) < beam_size and t < max_decoding_time_step:
            ...
            y_t_embed = self.model_embeddings_target(y_tm1)
            y_t_embed = torch.squeeze(y_t_embed, dim=1)
            ...
            #dec_state = (dec_state[0].permute(1, 0, 2), dec_state[1].permute(1, 0, 2))
            ...
            #h_tm1_1 = h_t[live_hyp_ids].permute(1, 0, 2)
            ...
            hyp_scores = torch.tensor(hyp_scores, dtype=torch.float, device=self.device)  # 将预测的分数存为张量

            #################！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！
            h_tm1 = (h_t[:,live_hyp_ids,:], cell_t[:,live_hyp_ids,:])###########大坑 原本是h_t[live_hyp_ids]=h_t[[0,0,0,]]维度都加到第一个上了，所以出错
            ###############！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！(sequence,batch,embedding_size)
