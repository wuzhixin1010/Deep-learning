...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        return [ [[self.char2id[char] for  char in '{' + words + '}'] for words in sentence] for sentence in sents ]
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        @returns sents_var: tensor of (batch_size, max_sentence_length, max_word_length)
        """
        ...
        sents_padded = pad_sents_char(word_ids, char_pad_token=self.char2id['<pad>'])
        sents_var = torch.tensor(sents_padded, dtype=torch.long, device=device)   #.permute(1,0,2)

        return sents_var  #(batch_size, max_sentence_length, max_word_length)
